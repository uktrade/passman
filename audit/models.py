...
def create_audit_event(user, action: Actions, description=None, secret=None, report_once=False):
    ...
    if report_once:
        report_window_start = timezone.now() - dt.timedelta(minutes=settings.AUDIT_EVENT_REPEAT_AFTER_MINUTES)
        if Audit.objects.filter(user=user, action=action.name, timestamp__gte=report_window_start).exists():
            return
